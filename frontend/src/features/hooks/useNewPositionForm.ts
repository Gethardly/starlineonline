...
import {type Device, type Pos, type Position, positionFormSchema} from "@/features/types";
...
export const useNewPositionForm = (isForm: boolean) => {
    ...
    const form = useForm<PositionFormData>({
        ...
        values: {
            ...
            device_id: selectedDevice?.deviceId.toString() ?? "",
    ...
    const getAddress = useCallback(async () => {
        try {
            if (!selectedDevice?.deviceId || devices.length === 0) {
    ...
    }, [devices.length, form, selectedDevice?.deviceId, selectedDevice?.pos.x, selectedDevice?.pos.y]);
    ...
    const onSelectedDeviceChange = async (val: string) => {
        const device = devices.find((d) => d.deviceId === val);

        if (device) {
            const {data: devicePosition} = await axiosApi.get(`/starline/device/${device.deviceId}/position`);
    ...
    const onPositionCreate = async (positionFormData: PositionFormData) => {
        try {
            const {data: devicePosition} = await axiosApi.get<Pos>(`starline/device/${positionFormData.device_id}/position`);
